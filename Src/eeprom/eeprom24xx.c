...
{
	if (HAL_I2C_IsDeviceReady(&_EEPROM24XX_I2C, 0xa0, 1000, _TIMEOUT) == HAL_OK)
		return true;
	else
		return false;
}
...
#if ((_EEPROM_SIZE_KBIT==1) || (_EEPROM_SIZE_KBIT==2))
	if(HAL_I2C_Mem_Write(&_EEPROM24XX_I2C,0xa0,Address,I2C_MEMADD_SIZE_8BIT,(uint8_t*)data,size_of_data,_TIMEOUT) == HAL_OK)
#else
...
{
#if ((_EEPROM_SIZE_KBIT==1) || (_EEPROM_SIZE_KBIT==2))
	if(HAL_I2C_Mem_Read(&_EEPROM24XX_I2C,0xa0,Address,I2C_MEMADD_SIZE_8BIT,(uint8_t*)data,size_of_data,_TIMEOUT) == HAL_OK)
#else
